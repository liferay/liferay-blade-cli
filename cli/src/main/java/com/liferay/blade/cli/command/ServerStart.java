 ...
 * @author David Truong
 */
public class ServerStart extends BaseCommand<ServerStartArgs> {
	...
	private void _commmandJBossWildfly(Path dir) throws Exception {
		...
		Process process = BladeUtil.startProcess(_blade, executable + debug, binPath.toFile(), enviroment, false);
	...
	private void _commmandTomcat(Path dir) throws Exception {
		...
		if (_args.isBackground() && _args.isTail()) {
			Process tailProcess = BladeUtil.startProcess(_blade, "tail -f catalina.out", logsPath.toFile(), enviroment);
