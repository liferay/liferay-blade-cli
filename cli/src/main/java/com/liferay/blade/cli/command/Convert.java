 ...
 */
public class Convert extends BaseCommand<ConvertArgs> {
	...
	private void _convertToServiceBuilderWarProject(File pluginDir) {
		...
		try {
			...
			new ConvertServiceBuilder(getBladeCLI(), convertServiceBuilderArgs).execute();
		}
		catch (Exception e) {
	...
	private void _convertToThemeBuilderWarProject(File themePlugin) {
		try {
			...
			Create createCommand = new Create(getBladeCLI());
	...
	private void _convertToThemeProject(File themePlugin) {
		try {
			new ConvertTheme(getBladeCLI(), getArgs()).execute();
		}
		catch (Exception e) {
			getBladeCLI().error("Error upgrading project %s\n%s", themePlugin.getName(), e.getMessage());
		}
