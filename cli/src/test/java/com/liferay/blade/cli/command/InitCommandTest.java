...
public class InitCommandTest {
	...
	public void testInitCommandGradleOption() throws Exception {
		...
		Assert.assertFalse(
			new File(
				gradleWorkspace, "pom.xml"
			).exists());

		Assert.assertTrue(
			new File(
				gradleWorkspace, "build.gradle"
			).exists());
	}
	...
	public void testInitInPluginsSDKDirectory() throws Exception {
		...
		Assert.assertTrue(
			new File(
				_workspaceDir, "themes"
			).exists());

		Assert.assertFalse(
			new File(
				_workspaceDir, "portlets"
			).exists());
		...
		Assert.assertFalse(
			new File(
				_workspaceDir, "build.properties"
			).exists());

		Assert.assertFalse(
			new File(
				_workspaceDir, "build.xml"
			).exists());

		Assert.assertTrue(
			new File(
				_workspaceDir, "plugins-sdk/build.properties"
			).exists());

		Assert.assertTrue(
			new File(
				_workspaceDir, "plugins-sdk/build.xml"
			).exists());
	}
	...
	public void testInitWithLiferayVersion70() throws Exception {
		...
		String properties = new String(
			Files.readAllBytes(
				new File(
					_workspaceDir, ".blade.properties"
				).toPath()));
	...
	public void testInitWithNameWorkspaceDirectoryHasFiles() throws Exception {
		...
		Assert.assertFalse(
			new File(
				_workspaceDir, "newproject/build.gradle"
			).exists());
	}
	...
	private void _makeSDK(File dir) throws IOException {
		...
		Assert.assertTrue(
			new File(
				dir, "build-common.xml"
			).createNewFile());
		Assert.assertTrue(
			new File(
				dir, "build-common-plugin.xml"
			).createNewFile());
	}
