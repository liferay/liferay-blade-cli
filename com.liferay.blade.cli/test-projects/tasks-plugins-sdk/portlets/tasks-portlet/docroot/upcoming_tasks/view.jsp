...
<div class="tasks-entries-container">
	<ul class="tasks-entries">
		...
		for (TasksEntry tasksEntry : taskEntries) {
			...
			if (TasksEntryPermission.contains(permissionChecker, tasksEntry, ActionKeys.UPDATE)) {
				LiferayPortletURL liferayPortletURL = liferayPortletResponse.createLiferayPortletURL(TasksPortletKeys.TASKS, PortletRequest.RENDER_PHASE);
	...
	<div class="view-all-tasks">
		...
		<%
		long tasksPlid = PortalUtil.getPlidFromPortletId(group.getGroupId(), TasksPortletKeys.TASKS);

		PortletURL portletURL = null;

		if (tasksPlid != 0) {
			portletURL = PortletURLFactoryUtil.create(request, TasksPortletKeys.TASKS, tasksPlid, PortletRequest.RENDER_PHASE);
		}
		%>
