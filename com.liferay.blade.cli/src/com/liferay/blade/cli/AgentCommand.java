...
public class AgentCommand {
	...
	public void _uninstall(AgentUninstallOptions options) throws Exception {
		...
		if (!modulesDir.exists()) {
			addError("Could not find modules dir at " + modulesDir.getAbsolutePath());
			return;
		}
		...
		for (File agentJar : list) {
			if (!agentJar.delete()) {
				addError("Could not delete remote agent jar " + agentJar.getAbsolutePath());
				return;
			}
		}
	...
	public void _install(AgentInstallOptions options) throws Exception {
		...
		if (options.repo() != null) {
			...
			props.put("locations", "https://bndtools.ci.cloudbees.com/job/bnd.master/866/artifact/dist/bundles/index.xml");
		}
		...
		if (options.version() != null) {
			files = repo.get("biz.aQute.remote.agent", options.version().toString());
		}
		else {
		...
		if (files == null || files.length == 0) {
			addError("Unable to find remote agent from remote repository. Please ensure internet connectivity.");
			return;
		}
		...
		if (options.liferayhome() != null) {
			...
			if (options.port() != -1) {
				try {
					...
					addError("Unable to connect to Liferay using JMX.  Please try again and specify the Liferay home folder using the -l option.");
					return;
				}
			}
			else {
				try {
					...
					addError("Unable to connect to Liferay using JDK attach API, please specify a port for JMX or specify Liferay home folder -l option.");
					return;
				}
			}
